...
export default function DataPage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Dữ liệu & nguồn</h1>
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm text-slate-600">
          <p><strong className="text-sky-700">Word:</strong> {SOURCES.word}</p>
          <p><strong className="text-sky-700">Excel:</strong> {SOURCES.excel} (25 sheet, 2.766 named range)</p>
          <p><strong className="text-sky-700">PPT:</strong> {SOURCES.ppt}</p>
        </CardContent>
      </Card>
        ...
        <CardContent className="overflow-x-auto">
          <table className="table-dark w-full text-left text-sm text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {MONTHS.map((m) => (
                <tr key={m}>
                  <td className="py-1.5 font-medium text-slate-900">{m}</td>
      ...
      <Card>
        ...
        <CardContent className="grid gap-2 sm:grid-cols-2 text-sm">
          {Object.entries(ASSUMPTIONS).map(([k, v]) => (
            <div key={k} className="flex justify-between rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
              <span className="text-slate-500">{k}</span>
              <span className="font-mono tabular-nums text-slate-700">{String(v)}</span>
            </div>
          ))}
        </CardContent>
      </Card>
