...
export function ActivityFeed() {
  ...
  return (
    <div className="cc-panel flex h-full flex-col rounded-xl border border-[var(--line)] bg-[var(--card)]">
      ...
      <div className="flex-1 space-y-0 overflow-y-auto p-2">
        {feed.map((f) => {
          const Icon = f.icon;
          return (
            <div
              key={f.id}
              className="flex gap-3 rounded-lg px-2 py-2.5 transition hover:bg-[var(--bg)]"
            >
              <div
                className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${toneCls[f.tone]}`}
              >
                <Icon className="h-3.5 w-3.5" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div className="text-[13px] font-bold leading-snug text-[var(--ink)]">
                    {f.title}
                  </div>
